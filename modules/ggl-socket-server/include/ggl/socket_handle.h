...
typedef struct {
    ...
    GgError (*on_register)(uint32_t handle, size_t index);
    GgError (*on_release)(uint32_t handle, size_t index);
    pthread_mutex_t mtx;
} GglSocketPool;
...
GgError ggl_socket_pool_register(GglSocketPool *pool, int fd, uint32_t *handle);
