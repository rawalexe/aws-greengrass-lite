...
static void new_client_available(
    ...
    if (client_fd == -1) {
        GG_LOGE("Failed to accept on socket %d: %d.", socket_fd, errno);
        return;
    }
    GG_CLEANUP_ID(client_fd_cleanup, cleanup_close, client_fd);

    GG_LOGD("Accepted new client %d.", client_fd);
    ...
    if (sys_ret == -1) {
        GG_LOGE("Failed to set send timeout on %d: %d.", client_fd, errno);
        return;
    }
    sys_ret = setsockopt(
    ...
    if (sys_ret == -1) {
        GG_LOGE("Failed to set receive timeout on %d: %d.", client_fd, errno);
        return;
    }

    uint32_t handle = 0;
    GgError ret = ggl_socket_pool_register(pool, client_fd, &handle);
    if (ret != GG_ERR_OK) {
        GG_LOGW("Closed new client %d due to max clients reached.", client_fd);
        return;
    }
    ...
    ret = gg_socket_epoll_add(epoll_fd, client_fd, handle);
    if (ret != GG_ERR_OK) {
        (void) ggl_socket_handle_close(pool, handle);
        GG_LOGE("Failed to register client %d with epoll.", client_fd);
        return;
    }
}
...
static void client_data_ready(
    ...
    GgError ret = client_ready(ctx, handle);
    if (ret != GG_ERR_OK) {
        (void) ggl_socket_handle_close(pool, handle);
    }
}

static GgError create_parent_dirs(char *path) {
    char *start = path;
...
static GgError configure_server_socket(
    ...
    GgError ret = create_parent_dirs(addr.sun_path);
    if (ret != GG_ERR_OK) {
        return ret;
    }
...
static GgError epoll_fd_ready(void *epoll_ctx, uint64_t data) {
    ...
    if (data == SERVER_FD_DATA) {
        ...
        GG_LOGE("Invalid data returned from epoll.");
        return GG_ERR_FAILURE;
    }
...
static GgBuffer split_fd_name_buffer(GgBuffer names, GgBuffer *name) {
    for (size_t i = 0; i < names.len; ++i) {
        if (names.data[i] == ':') {
            *name = gg_buffer_substr(names, 0, i);
            return gg_buffer_substr(names, i + 1, SIZE_MAX);
        }
    }
    *name = names;
    return gg_buffer_substr(names, names.len, SIZE_MAX);
}
...
static int inherit_socket_from_env(GgBuffer socket_name) {
    ...
    if ((pid_env != NULL) && (pid_env[0] != '\0')) {
        GG_LOGT("LISTEN_PID: %s", pid_env);
        int64_t pid;
        GgError err = gg_str_to_int64(gg_buffer_from_null_term(pid_env), &pid);
        if (err != GG_ERR_OK) {
            return -1;
        }
        if (pid != getpid()) {
            GG_LOGD("Socket was not intended for this PID.");
            return -1;
        }
    }
    ...
    GG_LOGT("LISTEN_FDS: %s", fds_env);
    int64_t fd_count;
    GgError err = gg_str_to_int64(gg_buffer_from_null_term(fds_env), &fd_count);
    if (err != GG_ERR_OK) {
        return -1;
    }
    // validate fd_count
    if ((fd_count < 0) || (fd_count > (INT_MAX - FD_SOCKET_ACTIVATION_START))) {
        GG_LOGD("Socket activation fd count not valid.");
        return -1;
    }
    ...
    if (getrlimit(RLIMIT_NOFILE, &rlim) != -1) {
        if ((uintmax_t) max_fd > rlim.rlim_cur) {
            GG_LOGD("Socket activation fd count too big.");
            return -1;
        }
    }
    ...
    GG_LOGT("LISTEN_FDNAMES: \"%s\"", names_env);

    // LISTEN_FDNAMES is a colon-delimited list of ASCII strings
    GgBuffer names = gg_buffer_from_null_term(names_env);
    for (int server_fd = FD_SOCKET_ACTIVATION_START; server_fd < max_fd;
        ...
        if (gg_buffer_eq(name, socket_name)) {
            GG_LOGT("Found socket.");
            if (!validate_server_socket(server_fd)) {
                GG_LOGD("Socket fd not open or not a socket.");
                return -1;
            }
            GG_LOGT("Configuring socket.");
...
GgError ggl_socket_server_listen(
    ...
    GG_CLEANUP(cleanup_close, epoll_fd);
    ...
    if (server_fd == -1) {
        GG_LOGD("Falling back to creating socket.");
        ...
        GG_LOGT("Listening on %.*s", (int) path.len, path.data);
    }
    GG_CLEANUP(cleanup_close, server_fd);

    ret = gg_socket_epoll_add(epoll_fd, server_fd, SERVER_FD_DATA);
    if (ret != GG_ERR_OK) {
        return ret;
    }

    if (ggl_socket_server_ext_handler != NULL) {
        ret = gg_socket_epoll_add(
    ...
    return gg_socket_epoll_run(epoll_fd, epoll_fd_ready, &server_ctx);
}
