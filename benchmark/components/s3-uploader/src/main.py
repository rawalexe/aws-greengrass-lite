...
class S3Uploader:
    ...
    def load_configuration(self):
        ...
        try:
            ...
            config["watchDirectory"] = os.environ.get('GG_WATCH_DIR',
                                                      config["watchDirectory"])
            config["s3Bucket"] = os.environ.get('GG_S3_BUCKET',
                                                config["s3Bucket"])
            config["s3Prefix"] = os.environ.get('GG_S3_PREFIX',
                                                config["s3Prefix"])
            config["uploadInterval"] = int(
                os.environ.get('GG_UPLOAD_INTERVAL', config["uploadInterval"]))
            config["deleteAfterUpload"] = os.environ.get(
                'GG_DELETE_AFTER', 'false').lower() == 'true'
            config["filePattern"] = os.environ.get('GG_FILE_PATTERN',
                                                   config["filePattern"])
            config["maxFileSize"] = int(
                os.environ.get('GG_MAX_FILE_SIZE', config["maxFileSize"]))
    ...
    def setup_s3_client(self):
        ...
        if BOTO3_AVAILABLE:
            ...
            logger.info(
                "Running in simulation mode - uploads will be logged only")
    ...
    def should_upload_file(self, file_path):
        ...
        try:
            ...
            if not fnmatch.fnmatch(path.name, self.config['filePattern']):
                logger.debug(
                    f"File {path.name} doesn't match pattern {self.config['filePattern']}"
                )
                return False
            ...
            if file_size > self.config['maxFileSize']:
                logger.warning(
                    f"File {path.name} too large ({file_size} bytes)")
                return False
                ...
                self.s3_client.upload_file(
                    ...
                    ExtraArgs={
                        'Metadata': {
                            ...
                            'source-device':
                            os.environ.get('AWS_IOT_THING_NAME', 'unknown')
                        }
                    })
                logger.info(
                    f"Uploaded {path.name} to s3://{self.config['s3Bucket']}/{s3_key}"
                )
            else:
                # Simulation mode
                logger.info(
                    f"[SIMULATION] Would upload {path.name} to s3://{self.config['s3Bucket']}/{s3_key}"
                )
    ...
    def run_with_polling(self):
        ...
        logger.info(
            f"Polling directory every {self.config['uploadInterval']} seconds")
